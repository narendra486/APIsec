...
def test_baseline_and_similarity_and_sql_error_detection():
    # Use a non-skipped host
    a = Analyzer('http://example.local/doLogin', skip_hosts=['demo.testfire.net'])
    ...
    with patch.object(a.session, 'post') as mock_post, patch.object(a.session, 'get') as mock_get:
        # baseline response
        mock_post.side_effect = [FakeResp(status=200, content=b'Normal page content'),
