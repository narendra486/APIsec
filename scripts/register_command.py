...
def register(command: str, name: str = None, auto_run: bool = True, patterns: List[str] = None) -> bool:
    data = load_commands()
    cmds = data.get('commands', [])
...
if __name__ == '__main__':
    ...
    ap.add_argument('--auto-run', action='store_true', help='Mark this command to auto-run on changes')
    ap.add_argument('--no-auto-run', action='store_true', help='Do not auto-run this command on changes')
    ap.add_argument('--patterns', help='Comma-separated glob patterns that trigger this command (e.g. "src/**,tests/**")')
    ap.add_argument('--run', action='store_true', help='Run the command immediately after registering')
    args = ap.parse_args()

    # default behavior: auto-run new commands unless --no-auto-run is provided
    auto_run_final = False if args.no_auto_run else True if args.auto_run or not args.no_auto_run else True
    registered = register(args.command, name=args.name, auto_run=auto_run_final, patterns=_parse_patterns(args.patterns))
    if args.run:
        env = os.environ.copy()
