"""
Advanced File Upload Bypass Testing Vectors
Double extension, MIME manipulation, ZIP slip
"""
...
FILE_UPLOAD_BYPASS_VECTORS = [
    TestVector(
        id="file-upload-ext-001",
        name="Double Extension Bypass",
        description="Tests if file extension validation can be bypassed with double extensions",
        vuln_type=VulnerabilityType.FILE_UPLOAD_EXTENSION_BYPASS,
        position=Position(
            type=PositionType.BODY_MULTIPART, name="filename", value_prefix="", value_suffix=""
        ),
        payload=PayloadTemplate(
            base="shell.php.jpg",
            variants=["shell.php.jpg", "shell.php%00.jpg", "shell.php;.jpg", "shell.php\x00.jpg"],
            encoding="none",
        ),
        expected_evidence=Evidence(
            indicators=["PHP file executed", "Extension bypass successful"],
            confidence=ConfidenceLevel.CRITICAL,
        ),
        sensitivity=SensitivityLevel.CRITICAL,
        remediation="Validate file type by content, not extension. Use allowlist.",
        references=["https://owasp.org/www-community/vulnerabilities/Unrestricted_File_Upload"],
        tags=["file-upload", "extension-bypass", "rce"],
    ),
    TestVector(
        id="file-upload-zip-001",
        name="ZIP Slip Path Traversal",
        description="Tests for path traversal in ZIP file extraction",
        vuln_type=VulnerabilityType.FILE_UPLOAD_ZIP_SLIP,
        position=Position(
            type=PositionType.BODY_MULTIPART, name="archive", value_prefix="", value_suffix=""
        ),
        payload=PayloadTemplate(base="malicious.zip", variants=[], encoding="binary"),
        expected_evidence=Evidence(
            indicators=["Files extracted outside target directory", "Path traversal successful"],
            confidence=ConfidenceLevel.CRITICAL,
        ),
        sensitivity=SensitivityLevel.CRITICAL,
        remediation="Validate ZIP entry paths. Reject paths with ../.",
        references=["https://cwe.mitre.org/data/definitions/22.html"],
        tags=["file-upload", "zip-slip", "path-traversal"],
    ),
    TestVector(
        ...
        position=Position(
            type=PositionType.HEADER, name="Content-Type", value_prefix="", value_suffix=""
        ),
        payload=PayloadTemplate(
            base="image/jpeg",
            variants=["image/jpeg", "image/png", "image/gif", "application/octet-stream"],
            encoding="none",
        ),
        expected_evidence=Evidence(
            indicators=["PHP file uploaded as image", "MIME bypass successful", "Script executed"],
    ...
    TestVector(
        ...
        position=Position(
            type=PositionType.BODY_MULTIPART, name="file", value_prefix="", value_suffix=""
        ),
        payload=PayloadTemplate(
        ...
        expected_evidence=Evidence(
            indicators=["Polyglot file executed", "Magic bytes bypass", "Code execution"],
            confidence=ConfidenceLevel.CRITICAL,
        ),
        sensitivity=SensitivityLevel.CRITICAL,
    ...
    TestVector(
        ...
        expected_evidence=Evidence(
            indicators=["Null byte truncation", "PHP file saved without .jpg extension", "RCE"],
            confidence=ConfidenceLevel.HIGH,
        ),
        sensitivity=SensitivityLevel.CRITICAL,
    ...
    TestVector(
        ...
        expected_evidence=Evidence(
            indicators=["File parsed as script", "Polyglot execution", "XSS or RCE"],
            confidence=ConfidenceLevel.HIGH,
        ),
        sensitivity=SensitivityLevel.CRITICAL,
    ...
    TestVector(
        ...
        expected_evidence=Evidence(
            indicators=["XSS executed from SVG", "Script in image file", "Alert displayed"],
            confidence=ConfidenceLevel.HIGH,
        ),
    ...
    TestVector(
        ...
        expected_evidence=Evidence(
            indicators=["File contents in response", "XXE exploitation", "Sensitive data leaked"],
            confidence=ConfidenceLevel.CRITICAL,
        ),
        sensitivity=SensitivityLevel.CRITICAL,
    ...
    TestVector(
        ...
        position=Position(
            type=PositionType.HEADER, name="Content-Length", value_prefix="", value_suffix=""
        ),
        payload=PayloadTemplate(
            base="100", variants=["100", "-1", "0", "999999999"], encoding="none"
        ),
        expected_evidence=Evidence(
            indicators=["Large file uploaded", "Size limit bypassed", "DoS condition"],
    ...
    TestVector(
        ...
        expected_evidence=Evidence(
            indicators=["Macro document uploaded", "Executable content allowed", "Malware risk"],
            confidence=ConfidenceLevel.HIGH,
        ),
        sensitivity=SensitivityLevel.HIGH,
    ...
    TestVector(
        ...
        payload=PayloadTemplate(
            base="42.zip", variants=["42.zip", "billion_laughs.xml"], encoding="binary"
        ),
        expected_evidence=Evidence(
            indicators=["Server resource exhaustion", "DoS via decompression", "Memory overflow"],
            confidence=ConfidenceLevel.HIGH,
        ),
        sensitivity=SensitivityLevel.HIGH,
    ...
    TestVector(
        ...
        payload=PayloadTemplate(
            base="malicious_symlink.tar", variants=["symlink.tar", "symlink.zip"], encoding="binary"
        ),
        expected_evidence=Evidence(
            indicators=["Symlink followed", "Arbitrary file read via symlink", "Path traversal"],
            confidence=ConfidenceLevel.HIGH,
        ),
        sensitivity=SensitivityLevel.CRITICAL,
    ...
    TestVector(
        ...
        position=Position(
            type=PositionType.HEADER, name="Content-Disposition", value_prefix="", value_suffix=""
        ),
        payload=PayloadTemplate(base="inline", variants=["inline", ""], encoding="none"),
        expected_evidence=Evidence(
    ...
    TestVector(
        id="file-upload-htaccess-001",
        name="Htaccess Upload for Code Execution",
        description="Tests if .htaccess files can be uploaded to enable script execution",
        vuln_type=VulnerabilityType.FILE_UPLOAD_EXECUTION,
        position=Position(
            type=PositionType.BODY_MULTIPART, name="file", value_prefix="", value_suffix=""
        ),
        payload=PayloadTemplate(
            base=".htaccess", variants=[".htaccess", "htaccess.txt"], encoding="none"
        ),
        expected_evidence=Evidence(
            indicators=[
                ".htaccess uploaded",
                "AddHandler directives active",
                "Code execution enabled",
            ],
            confidence=ConfidenceLevel.CRITICAL,
        ),
        sensitivity=SensitivityLevel.CRITICAL,
        remediation="Block .htaccess uploads. Disable AllowOverride. Use application-level config.",
        references=["https://owasp.org/www-community/vulnerabilities/Unrestricted_File_Upload"],
        tags=["file-upload", "htaccess", "code-execution"],
    ),
    TestVector(
        id="file-upload-eicar-001",
        name="EICAR Test File Upload",
        description="Tests if antivirus scanning is properly implemented",
        vuln_type=VulnerabilityType.FILE_UPLOAD_MALICIOUS_CONTENT,
        position=Position(
            type=PositionType.BODY_MULTIPART, name="file", value_prefix="", value_suffix=""
        ),
        payload=PayloadTemplate(
            base="X5O!P%@AP[4\\PZX54(P^)7CC)7}$EICAR-STANDARD-ANTIVIRUS-TEST-FILE!$H+H*",
            variants=["X5O!P%@AP[4\\PZX54(P^)7CC)7}$EICAR-STANDARD-ANTIVIRUS-TEST-FILE!$H+H*"],
            encoding="none",
        ),
        expected_evidence=Evidence(
            indicators=["EICAR file uploaded", "No antivirus scan", "Malware detection bypassed"],
            confidence=ConfidenceLevel.HIGH,
        ),
        sensitivity=SensitivityLevel.LOW,
        remediation="Implement antivirus scanning. Quarantine suspicious files. Scan before storage.",
        references=["https://www.eicar.org/download-anti-malware-testfile/"],
        tags=["file-upload", "antivirus", "malware-detection"],
    ),
]
