...
EXCEPTIONAL_CONDITIONS_VECTORS = [
    TestVector(
        ...
        expected_evidence=TestVectorEvidence(
            indicators=["Stack trace in response", "Sensitive info in error message"],
    ...
    TestVector(
        ...
        expected_evidence=TestVectorEvidence(
            indicators=["Access granted on error"], confidence=ConfidenceLevel.HIGH
        ),
        sensitivity=SensitivityLevel.HIGH,
    ...
    TestVector(
        ...
        expected_evidence=TestVectorEvidence(
            indicators=["Sensitive data in logs"], confidence=ConfidenceLevel.MEDIUM
        ),
        sensitivity=SensitivityLevel.HIGH,
    ...
    TestVector(
        ...
        expected_evidence=TestVectorEvidence(
            indicators=["No error reported, undefined behavior"], confidence=ConfidenceLevel.MEDIUM
        ),
    ...
    TestVector(
        ...
        payload=TestVectorPayload(
            base="Send malformed input to trigger exception and bypass validation."
        ),
        expected_evidence=TestVectorEvidence(
            indicators=["Validation bypass on error"], confidence=ConfidenceLevel.MEDIUM
        ),
        sensitivity=SensitivityLevel.MEDIUM,
