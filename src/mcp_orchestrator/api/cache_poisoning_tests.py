...
CACHE_POISONING_VECTORS = [
    TestVector(
        ...
        position=Position(type=PositionType.HEADER, name="X-Forwarded-Host", value_prefix="", value_suffix=""),
        ...
        expected_evidence=Evidence(
            indicators=["XSS payload in cached response", "Cache hit with malicious content", "X-Cache: HIT"],
    ...
    TestVector(
        ...
        vuln_type=VulnerabilityType.CACHE_POISONING,
        position=Position(type=PositionType.PATH, name="path", value_prefix="/account/", value_suffix=""),
        payload=PayloadTemplate(
            base="profile.css",
            variants=[
                "settings.js",
    ...
    TestVector(
        ...
        vuln_type=VulnerabilityType.CACHE_POISONING,
        position=Position(type=PositionType.HEADER, name="X-Original-URL", value_prefix="", value_suffix=""),
        payload=PayloadTemplate(
            base="/error",
            variants=[
                "/404",
        ...
        expected_evidence=Evidence(
            indicators=["Error page cached", "Service unavailable for all users", "X-Cache: HIT with error"],
            confidence=ConfidenceLevel.HIGH
        ),
        sensitivity=SensitivityLevel.HIGH,
    ...
    TestVector(
        ...
        vuln_type=VulnerabilityType.CACHE_POISONING,
        position=Position(type=PositionType.HEADER, name="X-Forwarded-Scheme", value_prefix="", value_suffix=""),
        payload=PayloadTemplate(
        ...
        expected_evidence=Evidence(
            indicators=["Split response cached", "Malicious content in cache", "CRLF injection successful"],
            confidence=ConfidenceLevel.HIGH
        ),
        sensitivity=SensitivityLevel.CRITICAL,
    ...
    TestVector(
        ...
        vuln_type=VulnerabilityType.CACHE_POISONING,
        position=Position(type=PositionType.HEADER, name="X-Forwarded-Proto", value_prefix="", value_suffix=""),
        payload=PayloadTemplate(
            base="https",
            variants=[
                "http",
        ...
        expected_evidence=Evidence(
            indicators=["Different cache key", "Cache bypass", "Protocol confusion", "Mixed content"],
            confidence=ConfidenceLevel.HIGH
        ),
