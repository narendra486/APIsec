...
WEBSOCKET_VECTORS = [
    ...
    TestVector(
        ...
        vuln_type=VulnerabilityType.WEBSOCKET_VULN,
        position=Position(
            type=PositionType.BODY_JSON, name="payload", value_prefix="", value_suffix=""
        ),
        payload=PayloadTemplate(
    ...
    TestVector(
        ...
        payload=PayloadTemplate(
            base="admin-protocol",
            variants=["debug-mode", "internal-api", "admin, unauthorized", "bypass-auth"],
            encoding="none",
        ),
        expected_evidence=Evidence(
            indicators=[
                "WebSocket connected without auth",
                "Admin protocol accepted",
                "Privileged access",
            ],
            confidence=ConfidenceLevel.HIGH,
        ),
